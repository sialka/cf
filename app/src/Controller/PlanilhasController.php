...
class PlanilhasController extends AppController {
    ...
    public function index() {
        ...
        $planilhas = $this->paginate($this->Planilhas->find('all')->contain(['Localidades','Users'])->where($_conditions['conditions']));
    ...
    public function add(){
        ...
        $now = Time::now();
        $hoje = $now->i18nFormat('yyyy-MM-dd');        
    ...
    public function edit($id = null){        
        ...
        $planilha = $this->Planilhas->get($id, ['contain' => ['Localidades', 'Ncs', 'Users']]); 
        //debug($planilha);        
            ...
            if($data){
                ...
                try {
                    ...
                    foreach ($data as $key => $linha) {
                        ...
                        $dt_lancamento = "00/00/0000";    
                        $dt_vencimento = "00/00/0000";    
                        $dt_pagamento = "00/00/0000";                            

                        if($linha->dt_lancamento != null){
                            $dt_lancamento = $linha->dt_lancamento->i18nFormat('dd/MM/yyyy');
                        }

                        if($linha->dt_vencimento != null){
                            $dt_vencimento = $linha->dt_vencimento->i18nFormat('dd/MM/yyyy');
                        }

                        if($linha->dt_pagamento != null){
                            $dt_pagamento = $linha->dt_pagamento->i18nFormat('dd/MM/yyyy');
                        }

                        $localidade = "Validar a Localidade";
                        $codigo = "Validar o Código";
                        $setor = "Validar o Setor";

                        if($linha->Localidades != null){
                            $setor = "BR ".$aevOptions['setores'][$linha->Localidades->setor];
                            $codigo = $linha->Localidades->codigo;
                            $localidade = $linha->Localidades->nome;
                        }
                        ...
                        $sheet->setCellValue("C{$column}", $localidade);
                        $sheet->setCellValue("D{$column}", $codigo);
                        $sheet->setCellValue("E{$column}", $setor);
                        $sheet->setCellValue("F{$column}", $dt_lancamento);
                        ...
                        $sheet->setCellValue("M{$column}", $dt_vencimento);
                        $sheet->setCellValue("N{$column}", $dt_pagamento);
